...
if (isset($_POST["id"]) && !empty($_POST["id"])) {
    ...
    if (empty($tanggal_err) && empty($kategori_err) && empty($instruksi_err) && empty($deskripsi_err) && empty($target_err) && empty($status_err)) {
        $sql = "UPDATE users SET tanggal = :tanggal, kategori = :kategori, instruksi = :instruksi, deskripsi = :deskripsi, target = :target, status = :status WHERE id = :id";
